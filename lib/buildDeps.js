...
function addModule(depTree, dirname, fileName, options, callback) {
    resolve(dirname, fileName, options, function (err, absoluteFileNames) {
        console.log(absoluteFileNames)
        if (!err) {
            if (depTree.modules[absoluteFileNames]) {
                callback(null, depTree.modules[absoluteFileNames].id);
            } else {
                const id = depTree.nextModuleId++;
                const absoluteLoaders = absoluteFileNames.split('!');
                const absoluteName = absoluteLoaders.pop();

                depTree.mapIdToName[id] = absoluteName;
                depTree.modules[absoluteName] = {
                    filename: absoluteName,
                    id,
                };

                function processJs(err, content) {
                    if (!err) {
                ...
                fs.readFile(absoluteName, function (err, content) {
                    execLoaders(absoluteLoaders, [content], absoluteName, options, processJs);
                })
            }
        } else {
            callback(err);
...
function execLoaders(loaders, content, fileName, options, callback) {
    const loaderFunctions = [];
    loaders.forEach(loader => {
        const loaderFunction = require(loader);
        loaderFunctions.push(loaderFunction);
    });
